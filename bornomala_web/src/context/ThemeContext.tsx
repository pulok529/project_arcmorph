...
export const SKIN_PRESETS: Record<string, Partial<ThemeConfig>> = {
  default: { theme: 'light', menuColor: 'dark', topbarColor: 'light', sidenavUser: false },
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [config, setConfig] = useState<ExtendedThemeConfig>(() => {
    const saved = localStorage.getItem('PACES_THEME_CONFIG');
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('PACES_THEME_CONFIG', JSON.stringify(config));
  }, [config]);
