...
export const IconsLucidePage: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedIcon, setCopiedIcon] = useState<string | null>(null);

  const icons: string[] = [
    'activity', 'airplay', 'alert-circle', 'alert-triangle', 'archive', 'arrow-down', 'arrow-left',
    'arrow-right', 'arrow-up', 'award', 'bar-chart', 'battery', 'bell', 'bluetooth', 'book',
    'bookmark', 'briefcase', 'calendar', 'camera', 'cast', 'check', 'chevron-down', 'chevron-left',
    'chevron-right', 'chevron-up', 'circle', 'clipboard', 'clock', 'cloud', 'code', 'compass',
    'cpu', 'credit-card', 'database', 'download', 'edit', 'eye', 'file', 'folder', 'gift',
    'globe', 'hard-drive', 'heart', 'help-circle', 'home', 'image', 'inbox', 'info', 'key'
  ];
  ...
  const handleCopy = (iconName: string) => {
    const snippet = `<iconify-icon icon="lucide:${iconName}"></iconify-icon>`;
    try {
  ...
  return (
    <div>
      <PageHeader title="Lucide Icons" category="Icons" />

      <div className="row">
        <div className="col-12">
          <Card title="Lucide Icons Database" badge={<span className="badge badge-soft-primary fs-xs">{filteredIcons.length} Icons</span>}>
              ...
              <div className="col-md-6 col-lg-4">
                <div className="input-group">
                  ...
                  <input
                    ...
                    placeholder="Search Lucide icons (e.g. user, home, arrow)..."
                    value={searchTerm}
            ...
            {copiedIcon && (
              <div className="alert alert-success d-flex align-items-center gap-2 mb-4 py-2" role="alert">
                ...
                <div>
                  Copied <strong>&lt;iconify-icon icon="lucide:{copiedIcon}"&gt;&lt;/iconify-icon&gt;</strong> to clipboard!
                </div>
              </div>
            )}
