...
export const IconsTablerPage: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [copiedIcon, setCopiedIcon] = useState<string | null>(null);

  const icons: string[] = [
    'home', 'user', 'settings', 'bell', 'search', 'mail', 'lock', 'eye', 'trash',
    'edit', 'plus', 'minus', 'check', 'x', 'arrow-right', 'arrow-left', 'arrow-up', 'arrow-down',
    'chevron-right', 'chevron-left', 'chevron-up', 'chevron-down', 'calendar', 'clock', 'camera',
    'folder', 'file', 'file-text', 'download', 'upload', 'cloud', 'heart', 'star', 'message',
    'phone', 'map-pin', 'chart-bar', 'chart-pie', 'chart-line', 'credit-card', 'shopping-cart',
    'database', 'server', 'cpu', 'shield', 'key', 'refresh', 'share', 'logout', 'help'
  ];
  ...
  const handleCopy = (iconName: string) => {
    const snippet = `<i className="ti ti-${iconName}"></i>`;
    try {
  ...
  return (
    <div>
      <PageHeader title="Tabler Icons" category="Icons" />

      <div className="row">
        <div className="col-12">
          <Card title="Tabler Icons Database" badge={<span className="badge badge-soft-primary fs-xs">{filteredIcons.length} Icons</span>}>
              ...
              <div className="col-md-6 col-lg-4">
                <div className="input-group">
                  ...
                  <input
                    ...
                    placeholder="Search Tabler icons (e.g. user, home, arrow)..."
                    value={searchTerm}
            ...
            {copiedIcon && (
              <div className="alert alert-success d-flex align-items-center gap-2 mb-4 py-2" role="alert">
                ...
                <div>
                  Copied <strong>&lt;i className="ti ti-{copiedIcon}"&gt;&lt;/i&gt;</strong> to clipboard!
                </div>
              </div>
            )}
            ...
            <div className="row g-3">
              {filteredIcons.map((iconName) => (
                <div key={iconName} className="col-6 col-sm-4 col-md-3 col-lg-2">
                  ...
                  >
                    <i className={`ti ti-${iconName} fs-28 mb-2 text-primary`}></i>
